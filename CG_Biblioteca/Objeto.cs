...
namespace CG_Biblioteca
{
  public class Objeto  //TODO: deveria ser uma class abstract ..??
  {
    ...
    public void Desenhar(Transformacao4D matrizGrafo, Camera camera)
    {
      DesenharInterno(matrizGrafo, camera, null);
    }

    public void Desenhar(Transformacao4D matrizGrafo, Objeto objetoSelecionado)
    {
      DesenharInterno(matrizGrafo, null, objetoSelecionado);
    }

    private void DesenharInterno(Transformacao4D matrizGrafo, Camera camera, Objeto objetoSelecionado)
    {
#if CG_OpenGL
      GL.PointSize(primitivaTamanho);
      ...
      if (paiRef != null)
      {
        ...
        Matrix4 matrizOpenTk = matrizGrafo.ObterDadosOpenTK();
        _shaderObjeto.SetMatrix4("transform", matrizOpenTk);
        if (camera != null)
        {
          _shaderObjeto.SetMatrix4("model", matrizOpenTk);
          _shaderObjeto.SetMatrix4("view", camera.GetViewMatrix());
          _shaderObjeto.SetMatrix4("projection", camera.GetProjectionMatrix());
        }
        _shaderObjeto.Use();
      ...
      {
        objetosLista[i].DesenharInterno(matrizGrafo, camera, objetoSelecionado);
      }
#endif
    }
